...
from ieisystem_inmanage_sdk.interface.Base import (Base, ascii2hex, hexReverse)
from ieisystem_inmanage_sdk.interface.NF5280M5 import NF5280M5
from ieisystem_inmanage_sdk.command import RestFunc, IpmiFunc
from ieisystem_inmanage_sdk.interface.ResEntity import (ResultBean, CapabilitiesBean, VncBean, VncSessionBean)
import os
import re
...
class NF5280M5_435(NF5280M5):
                ...
                if data["state"] == 0 and jj == ".json":
                    ...
                    while state == 0:
                        ...
                        if process_res == {}:
                            bmcres.State("Failure")
                            bmcres.Message(
                                ["get BMC cfg export progress error"])
                            break
                            ...
                            if "state" in data and data["state"] == 2:
                                ...
                                # download
                                download_res = RestFunc.downloadBMCcfgByRest(
                                    client, args.fileurl, filename)
                                if download_res == {}:
                                    bmcres.State("Failure")
                                ...
                                if "100" in data["progress"] or "complete" in data["progress"]:
                                    ...
                                    if download_res == {}:
                                        bmcres.State("Failure")
                                        bmcres.Message(
                                            ["download BMC cfg error"])
                                    elif download_res.get('code') == 0 and download_res.get('data') is not None:
                                        bmcres.State("Success")
                                        #bmcres.Message(["export BMC cfg success"])
                                        bmcres.Message(
                                            [download_res.get('data')])
                                    elif download_res.get('code') != 0 and download_res.get('data') is not None:
                                        bmcres.State("Failure")
                                        bmcres.Message(
                                            [download_res.get('data')])
                    ...
                    generate_res = RestFunc.generateBMCcfgByRest(
                        client, filename)
                    if generate_res == {}:
                        bmcres.State("Failure")
                        bmcres.Message(["cannot generate BMC cfg"])
                        ...
                        if "filename" in data:
                            ...
                            while state == 0:
                                ...
                                time.sleep(5)
                                # 循环查找直到完成
                                process_res = RestFunc.getBMCProgressByRest(
                                    client)
                                if process_res == {}:
                                    bmcres.State("Failure")
                ...
                while True:
                    ...
                    if process_res == {}:
                        if retrynum > 0:
                            ...
                            bmcres.State("Failure")
                            bmcres.Message(
                                ["import BMC cfg error, import progress is null"])
                            break
                    elif process_res.get('code') == 0 and process_res.get('data') is not None:
                        data = process_res.get('data')
                        if "completeCode" in data and data["completeCode"] == 0:
                            ...
                            errorlist = {
                                ...
                                2: "failed to create IPMI Session ",
                                3: "failed to parse Json file ",
                                4: "set operation over retry number ",
                                16: "service setting failed :Json format error ",
                                17: "service setting failed :Json array overbounds ",
                                18: "service setting failed: please enter valid id",
                                19: "service setting failed: please enter valid timeout ",
                                20: "service setting failed: configuration item missing ",
                                21: "service setting failed ",
                                32: "NTP setting failed: configuration item missing ",
                                33: "NTP setting failed: please enter the correct enable state ",
                                34: "NTP set failed: parse date failed ",
                                35: "NTP setting failed: parse time failed ",
                                36: "NTP setting failure: setting time failure ",
                                37: "NTP setting failed: setting UTC time zone failed ",
                                38: "NTP setting failed: failed to configure time zone ",
                                39: "NTP setting failed ",
                                48: "SMTP setting failed :Json format error ",
                                49: "SMTP setting failed :Json array out of bounds ",
                                50: "SMTP setting failed: configuration item missing ",
                                51: "SMTP setting failed: please enter valid network interface ",
                                52: "SMTP setting failed: please enter valid server IP address ",
                                53: "SMTP setting failed ",
                                64: "SNMP setting failure :Json format error ",
                                65: "SNMP setting fails :Json array overruns ",
                                66: "SNMP alarm IP setting fails ",
                                67: "SNMP warning email setting failed ",
                                68: "SNMP setting failed ",
                                80: "user setting failed :Json format error ",
                                81: "user setting failed :Json array overbounds ",
                                82: "user setting failed: add user failed ",
                                83: "user setting failed: modify user failed ",
                                84: "user setting fails: deletion fails ",
                                85: "failure of user setting: failure of obtaining user information ",
                                86: "user setting failed: please enter valid user ID",
                                87: "user setting failed: please enter valid user permissions ",
                                88: "user setting failed: user group name is empty ",
                                89: "user setting failed: user name is empty ",
                                90: "user setting fails: user name already exists ",
                                91: "user setting failed: password is empty ",
                                92: "user setting fails: password already exists ",
                                93: "user setting failed: old password is empty ",
                                94: "user setting failed: please enter a valid password change flag ",
                                95: "user setting fails: old password validation fails ",
                                96: "user setting failed: password length does not match ",
                                97: "user setting failed: password complexity not met ",
                                98: "user Settings fail: mailbox Settings fail ",
                                99: "user setting fails: mailbox format failed ",
                                100: "user setting failed: failed to add user to user group ",
                                112: "network setting failed :Json format error ",
                                113: "network setting failed :Json array overbounds ",
                                114: "network setting failed: please enter valid network interface ",
                                115: "network setting failed: please enter valid IP address ",
                                116: "network setting failed: please enter a valid subnet mask ",
                                117: "network setting failed: please enter valid default gateway ",
                                118: "network setting failed: please enter the correct enabling state ",
                                119: "network setting failed: configuration item missing ",
                                120: "network setting failed ",
                                121: "DNS setting failed ",
                                122: "network aggregation Settings failed ",
                                123: "network link setting failed ",
                                124: "network link setting failed: configuration item missing "}
                            bmcres.Message([errorlist[data["completeCode"]]])
                            break
                        else:
                            bmcres.State("Failure")
                            bmcres.Message(
                                ["import BMC cfg error, get import progress error"])
                            break
    ...
    def setvnc(self, client, args):
        ...
        if args.timeout is not None:
            if args.timeout < 60 or args.timeout > 1800 or args.timeout % 60 != 0:
                set_result.State("Failure")
                set_result.Message(
                    ["timeout is in 60-1800 and is mutiple of 60."])
                return set_result
            else:
        ...
        if args.timeout is not None or args.secureport is not None or args.nonsecureport is not None or args.enabled is not None:
            ...
            if args.timeout is None or args.secureport is None or args.nonsecureport is None or args.enabled is None:
                ...
                if Info_all:
                    if Info_all.get('code') == 0 and Info_all.get(
                        ...
                        set_result.State("Failure")
                        set_result.Message(
                            ["get current vnc status failed. " + Info_all.get('data')])
                        return set_result
                else:
                    set_result.State("Failure")
            ...
            if args.pwd is None:
                # 只设置其他项
                res = IpmiFunc.setM5VncByIpmi(
                    client, enabled, interface, nsp_hex, sp_hex, timeout)
                if res == {}:
                    set_result.State("Failure")
                    ...
                    set_result.State("Failure")
                    set_result.Message(
                        ["set vnc setting failed. " + res.get('code', '')])
            else:
                # 需要设置其他项和密码
                res = IpmiFunc.setM5VncByIpmi(
                    client, enabled, interface, nsp_hex, sp_hex, timeout)
                ...
                if res == {} or res_pwd == {}:
                    ...
                    set_result.State('Success')
                    set_result.Message(
                        ['set vnc password and setting success.'])
                elif res.get('code') == 0 and res_pwd.get('code') != 0:
                    set_result.State("Failure")
                    set_result.Message(
                        ["set vnc setting success,but failed to set password."])
                elif res.get('code') != 0 and res_pwd.get('code') == 0:
                    set_result.State("Failure")
                    set_result.Message(
                        ["set vnc password success,but failed to set setting."])
                else:
                    set_result.State("Failure")
            ...
            if res_pwd == {}:
                ...
                set_result.Message(
                    ["set vnc password failed. " + res_pwd.get('data', '')])
        else:
            set_result.State("Failure")
        ...
        if Info_all:
            if Info_all.get('code') == 0 and Info_all.get('data') is not None:
                ...
                if data.get('MaximumSessions', 0) == 'N/A':
                    ...
                    res.MaximumNumberOfSessions(
                        int(data.get('MaximumSessions', 0)))
                if data.get('ActiveSessions', 0) == 'N/A':
                    res.NumberOfActivatedSessions(0)
                else:
                    res.NumberOfActivatedSessions(
                        int(data.get('ActiveSessions', 0)))
                ...
                get_result.State('Failure')
                get_result.Message(
                    ['Failed to get vnc info. ' + Info_all.get('data')])
                return get_result
        else:
            get_result.State('Failure')
    ...
    def getvncsession(self, client, args):
        ...
        if res.get('code') == 0 and res.get('data') is not None:
            ...
            result.Message(
                ['failed to get vnc session info， ' + str(res.get('data'))])
        # logout
        RestFunc.logout(client)
        return result
    ...
    def delvncsession(self, client, args):
        ...
        if res.get('code') == 0 and res.get(
            ...
            if args.id is not None:
                ...
                flag = []
                for i in range(num):
                    res1 = RestFunc.deleteM5VncSessionByRest(
                        client, id_list[i])
                    if res1.get('code') == 0 and res1.get(
                            'data') is not None and res1.get('data').get('cc', 1) == 0:
                        continue
    ...
    def setbiospwd(self, client, args):
        ...
        if bios_version and 'code' in bios_version and bios_version['code'] == 0:
            ...
            version = bios_version['data'].get(
                'Version', 0).split('(')[0].split('.')
            if len(version) >= 3:
                if int(version[1]) != 3:
                    set_result.State('Failure')
                    set_result.Message(
                        ['current bios version is not support, please upgrade bios.'])
                    return set_result
            else:
                set_result.State('Failure')
        ...
        if Info_all:
            if Info_all.get('code') == 0:
                ...
                set_result.Message(
                    [
                        'Do not be in bios or in the process of switching on or off, and input right password format.' +
                        Info_all.get(
                            "code",
                            "")])
                return set_result
    ...
    def clearbiospwd(self, client, args):
        ...
        if Info_all:
            if Info_all.get('code') == 0:
                ...
                result.State('Failure')
                result.Message(
                    ['Failed to clear bios password. ' + Info_all.get('data', '')])
                return result
        else:
            result.State('Failure')
    ...
    def restorebios(self, client, args):
        ...
        if Info_all:
            if Info_all.get('code') == 0:
                ...
                result.State('Failure')
                result.Message(
                    ['Failed to restore bios. ' + Info_all.get('data', '')])
                return result
        else:
            result.State('Failure')
