...
import copy
import json
from ieisystem_inmanage_sdk.command import IpmiFunc
try:
...
def setPowerPolicyByRest(client, action):
    ...
    response = client.request("POST", "api/settings/power_policy", header, json=data)
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/settings/power_policy, response is none'
    elif response.status_code == 200:
...
def getM5VncSessionInfoByRest(client):
    ...
    response = client.request("GET", "api/settings/service-sessions?service_id=256", client.getHearder(), json=data_id,
                              data=None)
    if response is None:
        JSON['code'] = 1
...
def get5568M5VncSessionInfoByRest(client):
    ...
    response = client.request("GET", "api/settings/service-sessions?service_id=64", client.getHearder(), json=data_id,
                              data=None)
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/settings/service-sessions?service_id=256, response is none'
...
def deleteM5VncSessionByRest(client, id):
    JSON = {}
    response = client.request("DELETE", "api/settings/service-sessions/{0}".format(str(id)), client.getHearder(),
                              json=None,
                              data=None)
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface ' + 'api/settings/service-sessions/{0}'.format(
            str(id)) + ', response is none'
    elif response.status_code == 200:
        result = response.json()
        JSON['code'] = 0
        JSON['data'] = result
...
def ConfigurationsByRest(client):
    response = client.request("GET", "api/settings/media/remote/configurations", client.getHearder(), None, None, None,
                              None)
    JSON = {}
    if response is None:
        JSON['code'] = 1
...
def mountStartByRest(client, index, filename, type=1):
    ...
    response = client.request("POST", "api/settings/media/remote/start-media", client.getHearder(), data=None,
                              json=data)
    JSON = {}
    if response is None:
        JSON['code'] = 1
...
def setpwdByRest(client, args):
    data = {"UserOperation": 1, "password_old": args.oldupass, "access": args.access, "changepassword": 1,
            "confirm_password": args.upass, "email_format": "ami_format", "email_id": "", "fixed_user_count": 2,
            "id": args.userID, "kvm": 0, "name": args.uname, "group_name": args.group, "network_privilege": args.group,
            "password": args.upass, "password_size": "bytes_16", "privilege_limit_serial": "user", "snmp": 0,
            "snmp_access": "read_only", "snmp_authentication_protocol": "sha", "snmp_privacy_protocol": "des",
            "ssh_key": "Not Available", "ssh_status": 0, "vmedia": 0}
    JSON = {}
    if args.uname is None or args.uname == "":
        JSON['code'] = 1
        JSON['data'] = 'uname cannot be none'
...
def locateDiskByRestPMC(client, ctrlId, diskid, ledstate, duration):
    location_dict = {
        "StartLocate": 0,
        "StopLocate": 1
    }
    ...
    data = {
        'action': location_dict.get(ledstate),
        'ctrlId': ctrlId,
        'deviceId': diskid,
        'duration': duration,
        'vendor_type': 1
    ...
    header["Cookie"] = "" + header["Cookie"] + ";refresh_disable=1"
    r = client.request("PUT", "api/settings/raid_management/physical_start_stop_locate_cmd", data=None, json=data,
                       headers=header)
    if r is None:
        JSON["code"] = 1
        JSON[
            "data"] = 'Failed to api/settings/raid_management/physical_start_stop_locate_cmd, response is none'
    elif r.status_code == 200:
        JSON["code"] = 0
        JSON["data"] = ledstate
    else:
        JSON['code'] = 1
        JSON['data'] = formatError("api/settings/raid_management/physical_start_stop_locate_cmd", r)
    return JSON


def locateDiskByRestPMC_41401(client, ctrlId, diskid, ledstate, duration):
    location_dict = {
        "StartLocate": 0,
        "StopLocate": 1
    }
    ...
    header["Content-Type"] = "application/json"
    header["Cookie"] = "" + header["Cookie"] + ";refresh_disable=1"
    r = client.request("PUT", "api/raid/Locate_start_stop_PhysicalDisk", data=None, json=data,
                       headers=header)
    if r is None:
        JSON["code"] = 1
        JSON[
            "data"] = 'Failed to call BMC api/raid/Locate_start_stop_PhysicalDisk, response is none'
    elif r.status_code == 200:
        JSON["code"] = 0
        JSON["data"] = ledstate
    else:
        JSON['code'] = 1
        JSON['data'] = formatError("api/raid/Locate_start_stop_PhysicalDisk", r)
    return JSON
...
def statePDiskByRest(client, ctrlId, deviceId, option):
    data = {
        'ctrlId': ctrlId,
        'deviceId': deviceId,
        'data': option
    ...
    header["Content-Type"] = "application/json;charset=UTF-8"
    header["Cookie"] = "" + header["Cookie"] + ";refresh_disable=1"
    r = client.request("POST", "api/raid/setPDState", data=data, json=data, headers=header)
    if r is None:
        JSON["code"] = 1
...
def erasePDiskByRest(client, ctrlId, deviceId, option):
    data = {
        'ctrlId': ctrlId,
        'deviceId': deviceId,
        'data': option
    ...
    header["Content-Type"] = "application/json;charset=UTF-8"
    header["Cookie"] = "" + header["Cookie"] + ";refresh_disable=1"
    r = client.request("POST", "api/raid/erasePD", data=data, json=data, headers=header)
    if r is None:
        JSON["code"] = 1
...
def locatePDiskByRest(client, ctrlId, deviceId, option):
    ...
    header["Content-Type"] = "application/json;charset=UTF-8"
    header["Cookie"] = "" + header["Cookie"] + ";refresh_disable=1"
    r = client.request("POST", "api/raid/locatePD", data=data, json=data, headers=header)
    if r is None:
        JSON["code"] = 1
        JSON["data"] = 'Failed to call BMC interface api/raid/locatePD, response is none'
    elif r.status_code == 200:
...
def setSnmpM5ByRest(client, data):
    JSON = {}
    header = client.getHearder()
    header["X-Requested-With"] = "XMLHttpRequest"
    header["Content-Type"] = "application/json;charset=UTF-8"
    header["Cookie"] = "" + header["Cookie"] + ";refresh_disable=1"
    response = client.request("PUT", "api/settings/netsnmpconf", json=data, headers=header)
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/settings/netsnmpconf, response is none'
    elif response.status_code == 200:
...
def setAlertPoliciesByRest(client, data):
    JSON = {}
    response = client.request("PUT", "api/settings/pef/alert_policies/" + str(data['id']), client.getHearder(), json=data)
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/settings/pef/alert_policies, response is none'
    elif response.status_code == 200:
...
def deleteSystemLog(client, level):
    JSON = {}
    response = client.request("DELETE", "api/logs/system?level=" + str(level), client.getHearder(), None, None, None,
                              None)
    if response is None:
        JSON['code'] = 1
...
def setADgroupM6(client, adroup):
    ...
    if response is None:
        JSON['code'] = 1
        JSON['data'] = "Failed to call BMC interface api/settings/active-directory-users/" + str(
            id) + " ,response is none"
    elif response.status_code == 200:
        try:
            JSON["code"] = 0
...
def uploadBMCImageFile(client, updatefile):
    ...
    try:
        ...
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/maintenance/firmware, exception info about upload image'
        return JSON
    return JSON
...
def setBMCImageByRest(client, image):
    JSON = {}
    data = {
        "boot_image": image
    }
    response = client.request("PUT", "api/maintenance/dual_image_config", data=None, json=data, headers=client.getHearder())
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/maintenance/dual_image_config, response is none'
    elif response.status_code == 200:
...
def uploadBiosImageFile(client, updatefile):
    ...
    header["Connection"] = "keep-alive"
    header["X-CSRFToken"] = headers["X-CSRFToken"]
    header["Content-Length"] = "33554655"
    header["Cookie"] = "" + headers["Cookie"] + ";refresh_disable=1"
    JSON = {}
    if not os.path.exists(updatefile):
        JSON['code'] = 1
...
def updateBiosByRest(client, hasme):
    ...
    response = client.request("PUT", "api/maintenance/firmware/biosupgrade", client.getHearder(), json=data,
                              timeout=500)
    if response is None:
        JSON['code'] = 1
...
def setDNSRegByRest(client, data):
    JSON = {}
    response = client.request("POST", "api/set/dns/registration", client.getHearder(), json=data)
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC api/settings/dns/registration, response is none'
    elif response.status_code == 200:
        JSON['code'] = 0
        JSON['data'] = ""
    else:
        JSON['code'] = 1
        JSON['data'] = formatError("api/set/dns/registration", response)
    return JSON
...
def getNCSIByRest(client):
    JSON = {}
    response = client.request("GET", "api/settings/ncsi-interfaces-adaptive-support", client.getHearder(), None, None,
                              None, None)
    if response is None:
        JSON['code'] = 1
...
def getNCSIOptionByRest(client):
    JSON = {}
    response = client.request("GET", "api/settings/ncsi-interfaces-adaptive", client.getHearder(), None, None, None,
                              None)
    if response is None:
        JSON['code'] = 1
...
def getADByRest(client):
    JSON = {}
    response = client.request("GET", "api/settings/active-directory-settings", client.getHearder(), None, None, None,
                              None)
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/settings/active-directory-settings, response is none'
...
def addUserGroupByRest(client, name, pri):
    ...
    response = client.request("POST", "api/setusergroup", header, data=None, json=data)
    if response is None:
        JSON['code'] = 1
...
def getPowerConsumptionByRestM5(client):
    JSON = {}
    response = client.request("GET", "api/getpmbuspower", client.getHearder(), None, None, None, None)
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/getpmbuspower, response is none'
    elif response.status_code == 200:
        result = response.json()
        JSON['code'] = 0
        JSON['data'] = result
        ...
        JSON['code'] = 1
        JSON['data'] = formatError("api/getpmbuspower", response)
    return JSON
...
def resetBMC(client, type):
    ...
    response = client.request("POST", "api/diagnose/bmc-reset", data=data, json=data, headers=client.getHearder())
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/diagnose/bmc-reset ,response is none'
    elif response.status_code == 200:
        JSON["code"] = 0
        JSON["data"] = "reset success"
    else:
        JSON['code'] = 1
        JSON['data'] = formatError("api/diagnose/bmc-reset", response)
    return JSON
...
def getChassisLEDByRest(client):
    JSON = {}
    response = client.request("GET", "api/actions/chassis-led", client.getHearder(), None, None, None, None)
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/actions/chassis-led, response is none'
    elif response.status_code == 200:
        result = response.json()
        JSON['code'] = 0
        if result['led_status'] == 0:
            status = 'OFF'
        else:
            status = 'ON'
        JSON['data'] = {'UID': status}
    else:
        JSON['code'] = 1
        JSON['data'] = formatError("api/actions/chassis-led", response)
    return JSON
...
# 宕机截屏状态
def getAutoStateM5(client):
    JSON = {}
    response = client.request("GET", "api/diagnose/last-screen", client.getHearder())
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/diagnose/last-screen,response is none'
    elif response.status_code == 200:
        try:
            JSON["code"] = 0
            JSON["data"] = response.json()
            ...
            JSON['code'] = 1
            JSON['data'] = formatError("api/diagnose/last-screen", response)
    else:
        JSON['code'] = 1
        JSON['data'] = formatError("api/diagnose/last-screen", response)
    return JSON
...
# 宕机截屏状态开启关闭
# state 0关闭1开启
def changeAutoCaptureStateM5(client, state):
    JSON = {}
    data = {
        "enable": state,
        "id": 1
    }
    response = client.request("POST", "api/diagnose/last-screen", client.getHearder(), data=None, json=data)
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/diagnose/last-screen,response is none'
    elif response.status_code == 200:
...
def manualCaptureScreenM5(client, option):
    ...
    response = client.request("POST", "api/diagnose/trigger-capture", data=data, json=data, headers=client.getHearder())
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/diagnose/trigger-capture,response is none'
    elif response.status_code == 200:
        try:
            ...
            if option == 'capture':
                ...
                JSON["data"] = "Manual delete complete."
        except:
            JSON['code'] = 1
            JSON['data'] = formatError("api/diagnose/trigger-capture", response)
    else:
...
# 下载手动截图
def downloadManualCaptureScreenM5(client, filepath):
    JSON = {}
    url = 'https://' + client.host + '/bsod/screen0.jpeg'
    response = requests.get(url, verify=False)
    if response.status_code == 200:
        try:
            path = os.path.join(filepath, 'screen0.jpeg')
            ...
            return JSON
        JSON["code"] = 0
        JSON["data"] = "download screenshots complete, screenshots file: " + os.path.abspath(path)
    else:
        JSON['code'] = 1
...
# BMC自检结果
def getSelfTestByRest(client):
    JSON = {}
    response = client.request("GET", "api/diagnose/self-test", client.getHearder())
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/diagnose/self-test,response is none'
    elif response.status_code == 200:
        try:
            JSON["code"] = 0
            JSON["data"] = response.json()
            ...
            JSON['code'] = 1
            JSON['data'] = formatError("api/diagnose/self-test", response)
    else:
        JSON['code'] = 1
        JSON['data'] = formatError("api/diagnose/self-test", response)
    return JSON
...
def getNetworkBondByRest(client):
    JSON = {}
    response = client.request("GET", "api/settings/network-bond", client.getHearder())
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/settings/network-bond,response is none'
    elif response.status_code == 200:
        try:
            JSON["code"] = 0
            JSON["data"] = response.json()
            ...
            JSON['code'] = 1
            JSON['data'] = formatError("api/settings/network-bond", response)
    else:
        JSON['code'] = 1
        JSON['data'] = formatError("api/settings/network-bond", response)
    return JSON
...
def setNetworkBondByRest(client, data):
    JSON = {}
    response = client.request("PUT", "api/settings/network-bond", client.getHearder(), data=data, json=data)
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/settings/network-bond,response is none'
    elif response.status_code == 200:
...
def getNetworkLinkByRest(client):
    JSON = {}
    response = client.request("GET", "api/settings/network-link", client.getHearder())
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/settings/network-link,response is none'
    elif response.status_code == 200:
        try:
            JSON["code"] = 0
            JSON["data"] = response.json()
            ...
            JSON['code'] = 1
            JSON['data'] = formatError("api/settings/network-link", response)
    else:
        JSON['code'] = 1
        JSON['data'] = formatError("api/settings/network-link", response)
    return JSON
...
def setNetworkLinkByRest(client, data, id):
    JSON = {}
    header = client.getHearder()
    header["X-Requested-With"] = "XMLHttpRequest"
    header["Content-Type"] = "application/json;charset=UTF-8"
    header["Cookie"] = "" + header["Cookie"] + ";refresh_disable=1"
    response = client.request("PUT", "api/settings/network-link/" + str(id), header, data=None, json=data)
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/settings/network-link/' + str(id) + ',response is none'
    elif response.status_code == 200:
        try:
            JSON["code"] = 0
            JSON["data"] = response.json()
            ...
            JSON['code'] = 1
            JSON['data'] = formatError("api/settings/network-link/" + str(id), response)
    else:
        JSON['code'] = 1
        JSON['data'] = formatError("api/settings/network-link/" + str(id), response)
    return JSON
...
def setThresholdByRest(client, data):
    JSON = {}
    header = client.getHearder()
    header["X-Requested-With"] = "XMLHttpRequest"
    header["Content-Type"] = "application/json;charset=UTF-8"
    header["Cookie"] = "" + header["Cookie"] + ";refresh_disable=1"
    response = client.request("POST", "api/settings/setthreshold", header, data=data, json=data)
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/settings/setthreshold,response is none'
    elif response.status_code == 200:
        try:
            JSON["code"] = 0
            JSON["data"] = response.json()
            ...
            JSON['code'] = 1
            JSON['data'] = formatError("api/settings/setthreshold", response)
    else:
        JSON['code'] = 1
        JSON['data'] = formatError("api/settings/setthreshold", response)
    return JSON
...
# {BLADE_NUM: 1, DOMAIN_ID: 0, POLICY_ID: 3, TARGET_LIMIT: 1500,
# suspend:[{startTime: 0, endTime: 10, recurrence: 3}, {startTime: 220, endTime: 240, recurrence: 96}]}
# DOMAIN_ID: 0 system
# DOMAIN_ID: 1 cpu
def addPowerPolicy(client, data):
    JSON = {}
    response = client.request("GET", "api/chassis-status", client.getHearder(), None, None, None, None)
    if response is None:
        JSON['code'] = 1
        ...
        response = client.request("POST", "api/addpolicy", data=None, json=data, headers=client.getHearder())
        if response is None:
            JSON['code'] = 1
            JSON['data'] = 'Failed to call BMC interface api/getpolicy,response is none'
        elif response.status_code == 200:
...
# {BLADE_NUM: 1, POLICY_ID: 2, DOMAIN_ID: 0, action: 2}
def delPowerPolicy(client, data):
    JSON = {}
    response = client.request("POST", "api/setpolicy", data=None, json=data, headers=client.getHearder())
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/getpolicy,response is none'
    elif response.status_code == 200:
...
# {action: 1, BLADE_NUM: 1}
# action: 1 enable
# action: 0 disable
def setAllPowerPolicy(client, data):
    JSON = {}
    response = client.request("POST", "api/setallpolicy", data=None, json=data, headers=client.getHearder())
    if response is None:
        JSON['code'] = 1
...
# {BLADE_NUM: 1, POLICY_ID: 1, DOMAIN_ID: 0, action: 1}
# action: 1 enable
# action: 0 disable
# action: 2 delete
def setPowerPolicy(client, data):
    JSON = {}
    response = client.request("POST", "api/setpolicy", data=None, json=data, headers=client.getHearder())
    if response is None:
        JSON['code'] = 1
...
def getRangeWatts(client, domain):
    ...
    if response is None:
        ...
        try:
            range_serult = response.json()
            item_json = {}
            item_json['MaxPower'] = range_serult['MaxPower']
            item_json['MinPower'] = range_serult['MinPower']
            JSON["code"] = 0
            JSON["data"] = item_json
        except:
            JSON['code'] = 1
...
def getCPLDListByRest(client):
    JSON = {}
    response = client.request("GET", "api/cpld_list", headers=client.getHearder())
    if response is None:
        JSON['code'] = 1
        JSON['data'] = 'Failed to call BMC interface api/cpld_list,response is none'
    elif response.status_code == 200:
        try:
            JSON["code"] = 0
            JSON["data"] = response.json()
            ...
            JSON['code'] = 1
            JSON['data'] = formatError("api/cpld_list", response)
    else:
        JSON['code'] = 1
        JSON['data'] = formatError("api/cpld_list", response)
    return JSON
...
def uploadCPLDFileByRest(client, updatefile):
    JSON = {}
    header = client.getHearder()
    # 上传镜像到
    if not os.path.exists(updatefile):
    ...
    try:
        ...
        JSON["code"] = 1
        JSON["data"] = 'Upload file failed.'
        return JSON
    return JSON


def gupdateCPLDByRest(client, cpld_id):
    JSON = {}
    header = client.getHearder()
    header["X-Requested-With"] = "XMLHttpRequest"
    header["Content-Type"] = "application/json;charset=UTF-8"
    ...
    r = client.request("PUT", "api/maintenance/cpldflashimage", data=None, json=data, headers=header, timeout=500)
    if r is not None and r.status_code == 200:
        cpld_serult = r.json()
        if 'cpld_id' in cpld_serult:
            id = cpld_serult['cpld_id']
            if id == cpld_id:
                JSON["code"] = 0
                JSON["data"] = 'Upgrade complete, we recommand to AC Power Off and On.'
                return JSON
    JSON["code"] = 0
...
def locateLogicalDiskPMC(client, ctrlId, diskid, ledstate, duration):
    ...
    JSON = {}
    header = client.getHearder()
    r = client.request("PUT", "api/settings/raid_management/logical_start_stop_locate_cmd", data=None, json=data,
                       headers=header)
    if r is None:
        JSON["code"] = 1
        JSON[
            "data"] = 'Failed to call BMC interface api/settings/raid_management/logical_start_stop_locate_cmd, response is none'
    elif r.status_code == 200:
        JSON["code"] = 0
        JSON["data"] = ledstate
    else:
        JSON['code'] = 1
        JSON['data'] = formatError("api/settings/raid_management/logical_start_stop_locate_cmd", r)
    return JSON


def locateLogicalDiskPMC_41401(client, ctrlId, diskid, ledstate, duration):
    location_dict = {
        "StartLocate": 0,
        "StopLocate": 1
    }
    ...
    JSON = {}
    header = client.getHearder()
    r = client.request("PUT", "api/raid/Locate_start_stop_LogicalDisk", data=None, json=data,
                       headers=header)
    if r is None:
        JSON["code"] = 1
        JSON[
            "data"] = 'Failed to call BMC interface api/raid/Locate_start_stop_LogicalDisk, response is none'
    elif r.status_code == 200:
        JSON["code"] = 0
        JSON["data"] = ledstate
    else:
...
def deleteLogicalDiskPMC(client, ctrlId, diskid):
    ...
    JSON = {}
    header = client.getHearder()
    r = client.request("DELETE", "api/settings/raid_management/delete_virtual_drive", data=None, json=data,
                       headers=header)
    if r is None:
        JSON["code"] = 1
        JSON[
            "data"] = 'Failed to call BMC interface api/settings/raid_management/delete_virtual_drive, response is none'
    elif r.status_code == 200:
        JSON["code"] = 0
        JSON["data"] = ""
    else:
        JSON['code'] = 1
        JSON['data'] = formatError("api/settings/raid_management/delete_virtual_drive", r)
    return JSON


def deleteLogicalDiskPMC_41401(client, ctrlId, diskid):
    data = {
        'ctrlId': ctrlId,
        'deviceId': diskid,
        'vendor_type': 1
    ...
    JSON = {}
    header = client.getHearder()
    r = client.request("DELETE", "api/raid/Delete_LogicalDisk", data=None, json=data,
                       headers=header)
    if r is None:
        JSON["code"] = 1
        JSON[
            "data"] = 'Failed to call BMC interface api/raid/Delete_LogicalDisk, response is none'
    elif r.status_code == 200:
        JSON["code"] = 0
        JSON["data"] = ""
    else:
...
class NF5280M5_SensorDesc():
    ...
    def __init__(self):
        ...
        from ieisystem_inmanage_sdk.command.M5Log import eventLogString
        self.eventLogString = eventLogString.eventLogString

        from ieisystem_inmanage_sdk.command.M5Log import sensorSpecificEventStr
        self.sensorSpecificEventStr = sensorSpecificEventStr.sensorSpecificEventStr

        from ieisystem_inmanage_sdk.command.M5Log import biosPostEventStr
        self.biosPostEventStr = biosPostEventStr.biosPostEventStr

        from ieisystem_inmanage_sdk.command.M5Log import sensorEventStr
        self.sensorEventStr = sensorEventStr.sensorEventStr

        from ieisystem_inmanage_sdk.command.M5Log import commonInfoStr
...
def showSensorDesc(eventLogString, sensorSpecificEventStr, biosPostEventStr, sensorEventStr, curLang, commonInfoStr,
    ...
    if 0x0 <= item['record_type'] <= 0xBF:
        ...
        if item['gen_id1'] == 0x21 and item['sensor_type'] == 0xf:
            if getbits(item['event_data1'], 7, 6) == 0xC0 and (0 <= offset <= 2):
                if 2 == offset:
                    offset = 1
    ...
    elif 0xC0 <= item['record_type'] <= 0xDF:
        eventdesc = "OEM timestamped"

    elif 0xE0 <= item['record_type'] <= 0xFF:
